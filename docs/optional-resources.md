...
The `g` command dynamically imports released `@opentui/three` 0.4.5, `fontkit`, and the selected Quran font only after consent. It renders WebGPU pixels back into terminal character cells; it is not Kitty, SIXEL, or iTerm image-protocol support. Arabic is shaped RTL and converted from font outlines into live vector geometry rather than rasterizing a zoomed-out TUI: ayah mode extrudes the current verse, while page mode arranges up to fifteen bounded lines and brings the active line forward. The GPU viewport owns the reading area above the translation and below the header, with bright emissive glyphs composited in front of the terminal layout so React cannot erase them. `r` switches ayah/page mode and `f` cycles the Uthmani Hafs, IndoPak Nastaleeq, and QCF Tajweed faces supplied by Quran.com. Fonts are capped at 512 KiB, held in a two-entry memory cache, and released with the scene.

Exact page lines and IndoPak/Tajweed text come from compatible local `quran-script` plus `mushaf-layout` packs. An exact layout pack declares `compatibility.linesPerPage` and must supply every numbered line and the same complete word-coordinate set as its one attributed script pack; partial or mixed packs fall back instead of receiving an exact label. QCF Tajweed additionally requires page-numbered `code_v2` words, and every glyph layer must resolve against the font's embedded palette rather than guessed Unicode colors. Without local page data, Uthmani mode can request only the active ayah and its canonical page from the open Al Quran Cloud API, then flows those unchanged ayat to fit the terminal and labels the result `adaptive line flow`. The permission dialog makes that distinction explicit, and a missing source always offers a working Uthmani ayah reader instead of ending at an import instruction.
